...
@property (nonatomic ) NSInteger  aacBufferSize;
...
- (void)setUpEncoderFromSampleBuffer:(CMSampleBufferRef)sampleBuffer {
    ...
    outAudioStreamBasicDescription.mSampleRate = inAudioStreamBasicDescription.mSampleRate;   // 音频流, 在正常播放情况下的帧率。如果是压缩的格式，这个属性表示解压缩后的帧率。帧率不能为0。 在正常播放情况下的帧率，
    ...
    outAudioStreamBasicDescription.mBytesPerPacket = 0;    //每一个packet的音频数据大小。 如果的动态大小
    outAudioStreamBasicDescription.mFramesPerPacket = 1024;   //每个packet的帧数
    outAudioStreamBasicDescription.mBytesPerFrame = 0;    // 没帧的大小。
    ...
    OSStatus status = AudioConverterNewSpecific(&inAudioStreamBasicDescription, &outAudioStreamBasicDescription, 1 , description, &_audioConverter);    // 创建转化器
    if (status != 0) {
